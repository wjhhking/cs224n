...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = torch.nn.Conv1d(in_channels=embed_size, out_channels=embed_size, kernel_size=2, padding='same')
        self.encoder = torch.nn.LSTM(input_size=embed_size, hidden_size=hidden_size, num_layers=1, bias=True, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 3
        X = torch.nn.utils.rnn.pack_padded_sequence(input=X, lengths=source_lengths)
        enc_hiddens, (last_hidden, last_cell)  = self.encoder(X)
        enc_hiddens, _ = torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=False)
