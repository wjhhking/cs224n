...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            self.stack = self.stack[0:-2] + [self.stack[-2]]
        else:
            sys.exit("Error: Illegal transition " + transition)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        mini_batch = unfinished_parses[0:min(batch_size, len(unfinished_parses))]
        ...
        for unfinished_parse in unfinished_parses:
            if len(unfinished_parse.stack) == 1 and len(unfinished_parse.buffer) == 0:
                continue
            else:
                new_unfinished_parses.append(unfinished_parse)
        unfinished_parses = new_unfinished_parses

    for parse_result in partial_parses:
        dependencies.append(parse_result.dependencies)
