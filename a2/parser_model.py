...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = []
        # print('---w.size:', w.size(), w.size(0))
        # print('---n_features', self.n_features)
        # print('---view:', w.view(w.size(0), -1).size())
        # print('---embeddings.size:', self.embeddings.size())
        for i in w:
            # print('?', i, self.embeddings[i])
            x.append(self.embeddings[i])

        # x = self.embeddings[w].view(w.size(0), -1)
        # print('---embeddings[w]:', self.embeddings[w].size(), self.embeddings[w])
        # print('---x:', x.size(), x)

        ### END YOUR CODE
        return self.embeddings[w].view(w.size(0), -1)
    ...
    def forward(self, w):
        ...
        # print('===W.size:', self.embed_to_hidden_weight.size())
        # print('===U.size:', self.hidden_to_logits_weight.size())
        # print('===w.size:', w.size())
